...
describe('StudentHubPage', () => {
  it('bulk deletes selected visible students only after confirmation', async () => {
    ...
    server.use(http.get('/api/students', () => HttpResponse.json(envelope([student0001, student0002]))))
    renderHub()

    await user.click(await screen.findByLabelText('Select Student 0001'))
  ...
  it('clears active empty filters and requests the unfiltered list again', async () => {
    ...
    await user.type(screen.getByLabelText('Search'), 'Student 0001')
  ...
  it('debounces search for exactly 150ms and sends the instrument parameter', async () => {
    renderHub()
    await screen.findByText('Student 0001')
    listUrls = []
    vi.useFakeTimers()

    fireEvent.change(screen.getByLabelText('Search'), { target: { value: 'Student & Demo' } })
    ...
    expect(new URL(listUrls[0]).searchParams.get('query')).toBe('Student & Demo')
  ...
  it('opens a student through the accessible row link and saves only the touched field and version', async () => {
    ...
    const link = await screen.findByRole('link', { name: 'Open student Student 0001 (1001)' })
    link.focus()
  ...
  it('preserves touched draft through a 409 reload and retries with the fresh version', async () => {
    ...
    server.use(
      http.patch('/api/students/:studentId', async ({ request }) => {
        ...
        if (!conflicted) {
          conflicted = true
          detail = { ...detail, name_ch: '学生一修改', instructor: 'External edit' }
          version = 'v2'
    ...
    expect(await screen.findByLabelText('Chinese name')).toHaveValue('学生一修改')
    expect(instructor).toHaveValue('Instructor 0001')
    await user.click(screen.getByRole('button', { name: 'Save student' }))
  ...
  it('keeps the conflict and draft when reload fails, then retries with a fresh version', async () => {
    ...
    expect(screen.getByLabelText('Chinese name')).toHaveValue('学生一修改')
    expect(instructor).toHaveValue('Instructor 0001')
    await user.click(screen.getByRole('button', { name: 'Save student' }))
  ...
  it.each(['resolve', 'fail'] as const)('isolates a pending student A save from student B when it %s', async (outcome) => {
    ...
    server.use(
      http.get('/api/students', () => HttpResponse.json(envelope([detail, student0002]))),
      http.get('/api/students/:studentId', ({ params }) => {
        const record = params.studentId === '1002' ? student0002 : detail
      ...
      http.patch('/api/students/:studentId', async ({ params, request }) => {
        ...
        return HttpResponse.json({ ...envelope({ ...student0002, ...(body as object) }), workspace_version: 'b-v2' })
      }),
    )
    ...
    await user.type(instructorA, 'Instructor 0003')
    await user.click(screen.getByRole('button', { name: 'Save student' }))
    await waitFor(() => expect(requests).toHaveLength(1))

    await router.navigate('/students/1002')
    await waitFor(() => expect(screen.getByLabelText('English name')).toHaveValue('Student 0002'))
    ...
    if (outcome === 'resolve') {
      resolveStudentA?.(HttpResponse.json({ ...envelope({ ...detail, instructor: 'Instructor 0003' }), workspace_version: 'a-v2' }))
    } else {
    ...
    expect((client.getQueryData<{ data: typeof student0002 }>(['student', '1002']))?.data.student_id).toBe('1002')
    if (outcome === 'resolve') {
      await waitFor(() => expect((client.getQueryData<{ data: typeof student0001 }>(['student', '1001']))?.data.instructor).toBe('Instructor 0003'))
    }
