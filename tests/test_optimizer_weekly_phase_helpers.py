...
def test_schedule_weekly_blocks_hands_whole_group_to_resolve_when_no_single_room():
    lessons = [
        _lesson("l1", "Instructor 0004", "Piano", 9, 10),
        _lesson("l2", "Instructor 0004", "Piano", 10, 11),
    ]
    assigned = []
    unassigned = []

    logs = schedule_weekly_blocks(
        blocks=[{"inst": "Instructor 0004", "day": 1, "duration": 2, "lessons": lessons}],
        room_ids=["A", "B"],
        instructor_priority={},
    ...
    assert group_ids == {"weekly-group:Instructor 0004:1:l1"}
    assert all(payload["placement_group_size"] == 2 for payload, _code, _reason in unassigned)
    assert "    ⚠️ No single-room placement: Instructor 0004 (Day 1); handing 2 lessons to Resolve." in logs
...
def test_schedule_weekly_blocks_does_not_partially_fill_across_rooms():
    lessons = [
        _lesson("l1", "Instructor 0004", "Piano", 9, 10),
        _lesson("l2", "Instructor 0004", "Piano", 10, 11),
        _lesson("l3", "Instructor 0004", "Piano", 11, 12),
    ]
    assigned = []
    unassigned = []
    availability = {
    ...
    _schedule(
        blocks=[{"inst": "Instructor 0004", "day": 1, "duration": 3, "lessons": lessons}],
        room_ids=["R1", "R2"],
    ...
    assert {payload["placement_group_id"] for payload, _code, _reason in unassigned} == {
        "weekly-group:Instructor 0004:1:l1"
    }


def test_schedule_weekly_blocks_does_not_unassign_protected_lessons():
    lessons = [
        _lesson("l1", "Instructor 0004", "Piano", 9, 10, pinned=True),
        _lesson("l2", "Instructor 0004", "Piano", 10, 11),
    ]
    assigned = []
    unassigned = []
    ...
    assert unassigned[0][0]["placement_group_id"] == "weekly-group:Instructor 0004:1:l2"
